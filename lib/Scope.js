 ...
 * > **WARNING**
 * >
 * > In order for React types to be properly identified they must have a
 ...
 */
Scope.exposeAction = function(name, action) {
...
Scope.registerHTMLElementMethod = deprecate(
  'Scope.registerHTMLElementMethod has been renamed to Scope.exposeAction',
  Scope.exposeAction
);
...
Spt.findComponentByType = function(type, cond) {
  var instance = Selectors.findComponentByType(this.component, type, cond);
...
Spt.findByType = function(type, cond) {
  var node = Selectors.findByType(this.component, type, cond);
...
Spt.findAllByType = function(type, cond) {
  var nodes = Selectors.findAllByType(this.component, type, cond);
...
Spt.findBySelector = function(selector, cond, expectingMany) {
  var nodes = Selectors.findAll(selector, { container: this.node });
 ...
 * component.
 *
 * For example, a Modal library may render its content in another component that
 * can only be reached via an internal property. The warp routine allows you
 * to dig in and reach that property then re-form the scope to use it as its
 * component.
 *
 *     drill(component)
 *       .find(ReactModal)
...
Scope.prototype.warp = function(componentLocator, name) {
  const component = componentLocator(this.component);

  return new Scope(component, null, [ this.path, name ].join(' > '), this);
};
