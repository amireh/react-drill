...
describe('Scope with a stateless component', function() {
  ...
  const StatefulComponent = StatelessComponent => React.createClass({
    render() {
  ...
  context('with the component rendered somewhere down the tree', function() {
    ...
    it('does work', function() {
      drill(suite.component).find(StatelessButton).click()
    })
  })
})
...
describe('Scope', function() {
  ...
  describe('.exposeAction', function() {
    ...
    it('yields all additional args it was invoked with', function(done) {
      Scope.exposeAction('foo', function(node, arg1, arg2) {
        assert.equal(arg1, 1);
        assert.equal(arg2, 'a');
    ...
    it('invokes the method for every drilled node', function() {
      ...
      Scope.exposeAction('foo', function() {
        callCount++;
      });
